...
class GrpcTransport {
    ...
    ensureLoaded() {
        ...
        try {
            ...
            throw new Error('gRPC transport requires @grpc/grpc-js and @grpc/proto-loader as peer dependencies. ' +
     ...
     * @since 1.0.0
     */
    loadProto(protoPath, packageName) {
        this.ensureLoaded();
        const path = protoPath ?? this.cfg.protoPath;
        if (!path)
            throw new Error('No protoPath provided');
        ...
        if (pkg) {
            for (const segment of pkg.split('.')) {
                root = root[segment];
                if (!root)
                    throw new Error(`Package segment not found: ${segment}`);
            }
        }
    ...
    getService(serviceName) {
        ...
        if (!Ctor)
            throw new Error(`Service "${serviceName}" not loaded. Call loadProto() first.`);
        const creds = this.cfg.tls
            ? this.grpc.credentials.createSsl()
            : this.grpc.credentials.createInsecure();
        return new Ctor(this.cfg.endpoint, creds);
    }
    /**
     ...
     * @since 1.0.0
     */
    unary(service, method, request, opts = {}) {
        this.ensureLoaded();
        ...
        return new Promise((resolve, reject) => {
            fn.call(service, request, meta, callOpts, (err, res) => {
