...
exports.PRESET_REGISTRY = void 0;
 ...
 * ═══════════════════════════════════════════════════════════════ */
exports.PRESET_REGISTRY = (() => {
    const map = new Map();
    for (const raw of RAW_PRESETS) {
        const result = types_1.McpPresetSchema.safeParse(raw);
        if (!result.success) {
            throw new Error(`[McpPresets] Invalid preset "${raw.id ?? '?'}": ${result.error.message}`);
        }
...
function listPresets() {
    return [...exports.PRESET_REGISTRY.values()];
}
/** Lookup a single preset by id. Returns `undefined` if not found. */
function getPreset(id) {
    return exports.PRESET_REGISTRY.get(id);
}
