 ...
 * @since 1.3.0
 */
const DEFAULT_LATENCY_ESTIMATES = {
    jupiter: 2000,
    raydium: 1500,
    metaplex: 3000,
    native: 500,
...
class IntentPlanner {
    ...
    plan(intent) {
        ...
        for (const step of intent.steps) {
            if (!adjList.has(step.id))
                adjList.set(step.id, new Set());
        ...
        const dfs = (node) => {
            ...
            for (const neighbor of adjList.get(node) ?? []) {
                if (colors.get(neighbor) === GRAY) {
                    ...
                    throw new types_1.CyclicDependencyError(cycle.reverse());
                }
