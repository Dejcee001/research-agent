 ...
 * @since 1.3.0
 */
function resolveReferences(params, results, stepId) {
    const resolved = {};
    for (const [key, value] of Object.entries(params)) {
        if (typeof value === 'string' && value.startsWith('$')) {
            ...
            if (!stepResult) {
                throw new types_1.UnresolvedReferenceError(value, stepId);
            }
            if (stepResult.status !== 'completed' && stepResult.status !== 'simulated') {
                throw new types_1.UnresolvedReferenceError(`${value} (step '${refStepId}' has status '${stepResult.status}')`, stepId);
            }
            let val = stepResult.output;
            for (const field of fieldPath) {
                if (val === null || val === undefined) {
                    throw new types_1.UnresolvedReferenceError(`${value} — field '${field}' not found (value is ${val})`, stepId);
                }
                if (typeof val === 'object') {
                    val = val[field];
                }
                else {
                    throw new types_1.UnresolvedReferenceError(`${value} — cannot traverse '${field}' on non-object`, stepId);
                }
            }
            resolved[key] = val;
        }
            ...
            resolved[key] = resolveReferences(value, results, stepId);
        }
        else {
            resolved[key] = value;
        }
...
class IntentExecutor {
    ...
    async execute(plan, options) {
        if (!plan.isValid) {
            throw new types_1.IntentError(`Cannot execute invalid plan: ${plan.errors.join(', ')}`, 'INVALID_PLAN');
        }
        ...
        for (const step of plan.steps) {
            if (!levels.has(step.executionOrder))
                levels.set(step.executionOrder, []);
        ...
        for (const level of sortedLevels) {
            ...
            for (const step of stepsAtLevel) {
                ...
                if (result.status === 'completed' || result.status === 'simulated') {
                    ...
                    await this.config.onStepError?.(step.id, new types_1.IntentError(result.error ?? 'Unknown error', 'STEP_FAILED', step.id));
                }
            }
     ...
     * @since 1.3.0
     */
    async executeAtomic(plan, options) {
        return this.execute(plan, { ...options, atomic: true });
    }
    /* ── Single step execution ── */
    async executeStep(step, results, simulate) {
        const start = Date.now();
        try {
            // Resolve references
            const resolvedParams = resolveReferences(step.params, results, step.id);
            // Find the tool
            const protocolTools = this.config.protocolTools[step.protocol];
            if (!protocolTools) {
                throw new types_1.IntentError(`No tools registered for protocol '${step.protocol}'`, 'PROTOCOL_NOT_FOUND', step.id);
            }
            const tool = protocolTools[step.method];
            if (!tool) {
                throw new types_1.IntentError(`Method '${step.method}' not found in protocol '${step.protocol}'`, 'METHOD_NOT_FOUND', step.id);
            }
            // Execute with timeout
            const timeoutMs = step.timeoutMs ?? this.defaultTimeout;
            const rawOutput = await Promise.race([
                tool.invoke(resolvedParams),
                new Promise((_, reject) => setTimeout(() => reject(new types_1.IntentError(`Step '${step.id}' timed out after ${timeoutMs}ms`, 'TIMEOUT', step.id)), timeoutMs)),
            ]);
            // Parse output
            let output;
            try {
...
exports.IntentExecutor = IntentExecutor;
