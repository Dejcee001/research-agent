...
exports.FacilitatorClient = exports.SettleError = exports.VerifyError = exports.FacilitatorError = void 0;
 ...
 * @since 1.0.0
 */
class VerifyError extends FacilitatorError {
    constructor(statusCode, response) {
        super(`Payment verification failed: ${response.invalidReason ?? 'unknown'}`, '/verify', statusCode, response);
 ...
 * @since 1.0.0
 */
class SettleError extends FacilitatorError {
    constructor(statusCode, response) {
        super(`Payment settlement failed: ${response.errorReason ?? 'unknown'}`, '/settle', statusCode, response);
        this.name = 'SettleError';
    }
}
exports.SettleError = SettleError;
/* ═══════════════════════════════════════════════════════════════
 *  FacilitatorClient
 * ═══════════════════════════════════════════════════════════════ */
/**
...
class FacilitatorClient {
    ...
    constructor(config) {
        ...
        if (typeof config === 'string' && Object.values(types_1.KnownFacilitator).includes(config)) {
            this.knownFacilitator = config;
            const resolved = (0, registry_1.resolveKnownFacilitator)(config);
    ...
    async supported() {
        ...
        if (res.status !== 200) {
            throw new FacilitatorError(`Failed to fetch supported kinds: ${res.status} ${res.statusText}`, '/supported', res.status);
        }
        return await res.json();
    }
    /* ── Helpers ─────────────────────────────────────────────── */
    async getHeaders(endpoint) {
        if (!this.config.createAuthHeaders)
            return {};
    ...
    async fetch(url, init) {
        ...
        try {
            ...
            if (err.name === 'AbortError') {
                throw new FacilitatorError(`Facilitator request timed out after ${this.config.timeoutMs}ms`, url, 0);
            }
            throw err;
        }
        finally {
