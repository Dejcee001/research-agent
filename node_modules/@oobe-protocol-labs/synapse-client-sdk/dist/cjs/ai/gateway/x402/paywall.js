...
class X402Paywall {
    ...
    supportedKindsCacheTtlMs = 60_000; // 1 min
    /**
    ...
    constructor(config) {
        this.config = config;
        this.facilitator = new facilitator_1.FacilitatorClient(config.facilitator);
    }
     ...
     * @since 1.0.0
     */
    async processRequest(method, headers) {
        if (!this.config.enabled) {
            return { type: 'no-payment-needed' };
        }
        // Check for incoming payment
        const paymentSignatureRaw = headers[types_1.X402_HEADER_PAYMENT_SIGNATURE]
            ?? headers[types_1.X402_HEADER_PAYMENT_SIGNATURE.toLowerCase()]
        ...
        const paymentPayload = decodePaymentHeader(paymentSignatureRaw);
     ...
     * @since 1.0.0
     */
    async settleAfterResponse(paymentPayload, requirements) {
        try {
     ...
     * @since 1.0.0
     */
    async buildRequirementsWithFeePayer(method) {
    ...
    /* ── Build 402 response ──────────────────────────────────── */
    async buildPaymentRequired(method, error) {
        ...
        const body = {
            x402Version: types_1.X402_VERSION,
            error,
            resource,
        ...
        return {
            ...
            statusCode: types_1.X402_STATUS_CODE,
            headers: {
                [types_1.X402_HEADER_PAYMENT_REQUIRED]: headerValue,
