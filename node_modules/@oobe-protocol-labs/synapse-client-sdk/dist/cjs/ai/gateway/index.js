...
exports.createMultiProtocolMonetizedTools = exports.createMonetizedTools = exports.MonetizeError = exports.MemoryAdapter = exports.AgentRegistry = exports.ToolMarketplace = exports.ResponseValidator = exports.DEFAULT_TIERS = exports.PricingEngine = exports.CallLimitExceededError = exports.SessionExpiredError = exports.RateLimitExceededError = exports.BudgetExhaustedError = exports.SessionError = exports.AgentSession = exports.createAgentId = exports.AgentGateway = exports.IntentVerificationError = exports.MaxSessionsError = exports.SessionNotFoundError = exports.GatewayError = void 0;
...
const paywall_1 = require("./x402/paywall");
 ...
 * @since 1.0.0
 */
class MaxSessionsError extends GatewayError {
    /**
 ...
 * @since 1.0.0
 */
class IntentVerificationError extends GatewayError {
    /**
...
exports.IntentVerificationError = IntentVerificationError;
/* ═══════════════════════════════════════════════════════════════
 *  AgentGateway
 * ═══════════════════════════════════════════════════════════════ */
/**
...
class AgentGateway {
    ...
    constructor(client, config) {
        ...
        this.pricing = new pricing_1.PricingEngine(config.defaultTiers, config.methodTiers);
        this.validator = new validator_1.ResponseValidator(config);
        this.marketplace = new marketplace_1.ToolMarketplace();
        // Initialize x402 paywall (seller side) if configured
        this.paywall = config.x402
            ? new paywall_1.X402Paywall(config.x402)
            : null;
        // Initialize x402 client (buyer side) if configured
        this.x402Client = config.x402Client
            ? new client_1.X402Client(config.x402Client)
            : null;
    }
    /* ═══════════════════════════════════════════════════════════
     *  Identity
     ...
     * @since 1.0.0
     */
    openSession(intent, opts = {}) {
        // ── Concurrency limit
        const maxSessions = this.config.maxConcurrentSessions ?? 100;
        const activeSessions = [...this.sessions.values()].filter(s => s.status === 'active' || s.status === 'pending' || s.status === 'paused').length;
        ...
        const session = new session_1.AgentSession(intent, tier, this.agentId, ttl);
     ...
     * @since 1.0.0
     */
    settleSession(sessionId, txSignature = null) {
     ...
     * @since 1.0.0
     */
    async executeBatch(sessionId, calls) {
        const results = [];
        for (const call of calls) {
            results.push(await this.execute(sessionId, call.method, call.params));
        }
        return results;
    }
    /* ═══════════════════════════════════════════════════════════
     ...
     * @since 1.0.0
     */
    async processX402Request(method, headers) {
        if (!this.paywall) {
     ...
     * @since 1.0.0
     */
    async settleX402Payment(paymentPayload, requirements) {
        if (!this.paywall) {
    ...
    async executeWithX402(sessionId, method, params = [], incomingHeaders = {}, opts = {}) {
        ...
        if (sessionId) {
            ...
            const result = await this.transport.request(method, params);
            const latencyMs = Math.round(performance.now() - start);
            attested = await this.validator.wrapResult(result, '', method, params, 0, latencyMs, 0, false);
            this.totalCallsServed++;
            this.pricing.reportLatency(latencyMs);
        }
        pushStep({
     ...
     * @since 1.0.0
     */
    async executeRemoteX402(url, method, params = []) {
        if (!this.x402Client) {
        ...
        if (!response.ok && response.status !== 402) {
            throw new GatewayError(`Remote RPC failed: ${response.status} ${response.statusText}`, 'REMOTE_RPC_ERROR');
        }
        const json = await response.json();
        if (json.error) {
            throw new GatewayError(`Remote RPC error: ${JSON.stringify(json.error)}`, 'REMOTE_RPC_ERROR');
        }
        if (payment) {
        ...
        return { result: json.result, payment };
    }
    /**
     ...
     * @since 1.0.0
     */
    publishBundle(name, methods, tiers, description) {
        const bundle = {
            id: (0, crypto_1.randomUUID)(),
            name,
     ...
     * @since 1.0.0
     */
    async createGatewayTools(sessionId) {
        this.getSessionOrThrow(sessionId);
        // Lazy import to keep gateway usable without langchain installed
        const { tool: lcTool } = await Promise.resolve().then(() => __importStar(require('@langchain/core/tools')));
        const { agentRpcMethods } = await Promise.resolve().then(() => __importStar(require('../tools/zod')));
        const tools = [];
        for (const method of agentRpcMethods) {
            const gateway = this;
            const sid = sessionId;
            const t = lcTool(async (input) => {
                try {
 ...
 * @since 1.0.0
 */
function createAgentGateway(client, config) {
    return new AgentGateway(client, config);
}
var types_1 = require("./types");
Object.defineProperty(exports, "createAgentId", { enumerable: true, get: function () { return types_1.AgentId; } });
var session_2 = require("./session");
Object.defineProperty(exports, "AgentSession", { enumerable: true, get: function () { return session_2.AgentSession; } });
Object.defineProperty(exports, "SessionError", { enumerable: true, get: function () { return session_2.SessionError; } });
Object.defineProperty(exports, "BudgetExhaustedError", { enumerable: true, get: function () { return session_2.BudgetExhaustedError; } });
Object.defineProperty(exports, "RateLimitExceededError", { enumerable: true, get: function () { return session_2.RateLimitExceededError; } });
Object.defineProperty(exports, "SessionExpiredError", { enumerable: true, get: function () { return session_2.SessionExpiredError; } });
Object.defineProperty(exports, "CallLimitExceededError", { enumerable: true, get: function () { return session_2.CallLimitExceededError; } });
var pricing_2 = require("./pricing");
Object.defineProperty(exports, "PricingEngine", { enumerable: true, get: function () { return pricing_2.PricingEngine; } });
Object.defineProperty(exports, "DEFAULT_TIERS", { enumerable: true, get: function () { return pricing_2.DEFAULT_TIERS; } });
...
/* ── Monetization Bridge ───────────────────────────────────── */
var monetize_1 = require("./monetize");
Object.defineProperty(exports, "MonetizeError", { enumerable: true, get: function () { return monetize_1.MonetizeError; } });
Object.defineProperty(exports, "createMonetizedTools", { enumerable: true, get: function () { return monetize_1.createMonetizedTools; } });
Object.defineProperty(exports, "createMultiProtocolMonetizedTools", { enumerable: true, get: function () { return monetize_1.createMultiProtocolMonetizedTools; } });
