...
exports.AgentSession = exports.CallLimitExceededError = exports.SessionExpiredError = exports.RateLimitExceededError = exports.BudgetExhaustedError = exports.SessionError = void 0;
 ...
 * @since 1.0.0
 */
class BudgetExhaustedError extends SessionError {
    /**
 ...
 * @since 1.0.0
 */
class RateLimitExceededError extends SessionError {
    /**
     ...
     * @since 1.0.0
     */
    constructor(sessionId, retryAfterMs) {
 ...
 * @since 1.0.0
 */
class CallLimitExceededError extends SessionError {
    /**
...
exports.CallLimitExceededError = CallLimitExceededError;
/* ═══════════════════════════════════════════════════════════════
 *  AgentSession
 * ═══════════════════════════════════════════════════════════════ */
/**
...
class AgentSession {
     ...
     * @since 1.0.0
     */
    constructor(intent, tier, sellerId, ttl = 3600) {
        const now = Date.now();
        this.state = {
    ...
    preCall(method) {
        // ── Status check
        if (this.state.status !== 'active') {
            throw new SessionError(`Session is '${this.state.status}', cannot make calls`, 'INVALID_STATE', this.state.id);
        }
        // ── TTL check
        if (this.state.ttl > 0) {
     ...
     * @since 1.0.0
     */
    postCall(method, cost) {
        const now = Date.now();
        // Deduct
        this.state.budgetRemaining -= cost;
        this.state.callsMade += 1;
        if (this.state.callsRemaining !== -1)
            this.state.callsRemaining -= 1;
