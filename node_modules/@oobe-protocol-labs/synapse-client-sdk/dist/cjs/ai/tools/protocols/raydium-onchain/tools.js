...
exports.raydiumOnchainMethodNames = exports.raydiumOnchainMethods = exports.RAYDIUM_PROGRAM_IDS = void 0;
exports.createRaydiumOnchainTools = createRaydiumOnchainTools;
/**
...
const RAYDIUM_ID_REVERSE = new Map();
for (const [key, val] of Object.entries(exports.RAYDIUM_PROGRAM_IDS)) {
    RAYDIUM_ID_REVERSE.set(val.id, { key, name: val.name });
}
/* ═══════════════════════════════════════════════════════════════
 *  Execution dispatcher — on-chain via RPC transport
 * ═══════════════════════════════════════════════════════════════ */
function createRaydiumOnchainExecutor(transport) {
...
async function executeFetchAccount(transport, input) {
    const cfg = {};
    if (input.encoding)
        cfg.encoding = input.encoding;
    else
        cfg.encoding = 'base64';
    if (input.commitment)
        cfg.commitment = input.commitment;
    if (input.dataSlice)
        cfg.dataSlice = input.dataSlice;
    return transport.request('getAccountInfo', [input.address, cfg]);
}
async function executeSearchProgramAccounts(transport, input) {
    const programKey = input.program;
    const entry = exports.RAYDIUM_PROGRAM_IDS[programKey];
    if (!entry)
        throw new Error(`Unknown Raydium program: ${programKey}`);
    const cfg = {};
    if (input.encoding)
        cfg.encoding = input.encoding;
    else
    ...
    if (input.dataSlice)
        cfg.dataSlice = input.dataSlice;
    if (input.filters)
        cfg.filters = input.filters;
    const accounts = await transport.request('getProgramAccounts', [entry.id, cfg]);
    const maxResults = input.maxResults ?? accounts.length;
...
async function executeGetTransactionHistory(transport, input) {
    const cfg = {};
    if (input.limit)
        ...
        cfg.limit = 20;
    if (input.before)
        cfg.before = input.before;
    if (input.until)
        cfg.until = input.until;
    if (input.commitment)
        cfg.commitment = input.commitment;
    return transport.request('getSignaturesForAddress', [input.address, cfg]);
}
async function executeInspectTransaction(transport, input) {
    const cfg = {
        encoding: 'jsonParsed',
    ...
    if (input.commitment)
        cfg.commitment = input.commitment;
    const tx = await transport.request('getTransaction', [input.signature, cfg]);
    if (!tx) {
...
async function executeGetPoolState(transport, input) {
    const poolAddress = input.poolAddress;
    const cfg = {};
    if (input.encoding)
        cfg.encoding = input.encoding;
    else
    ...
    if (input.commitment)
        cfg.commitment = input.commitment;
    const result = await transport.request('getAccountInfo', [poolAddress, cfg]);
    if (!result.value) {
        return {
            address: poolAddress,
            owner: null,
...
async function executeGetMultiplePoolStates(transport, input) {
    const poolAddresses = input.poolAddresses;
    const cfg = {};
    if (input.encoding)
        cfg.encoding = input.encoding;
    else
    ...
    if (input.commitment)
        cfg.commitment = input.commitment;
    const result = await transport.request('getMultipleAccounts', [poolAddresses, cfg]);
    let found = 0;
    let missing = 0;
...
async function executeGetPoolsByMint(transport, input) {
    const programKey = input.program;
    const entry = exports.RAYDIUM_PROGRAM_IDS[programKey];
    if (!entry)
        throw new Error(`Unknown Raydium program: ${programKey}`);
    const mint = input.mint;
    const mintOffset = input.mintOffset;
...
async function executeGetLPPositions(transport, input) {
    ...
    if (input.commitment)
        cfg.commitment = input.commitment;
    const result = await transport.request('getTokenAccountsByOwner', [owner, filter, cfg]);
...
async function executeGetFarmPositions(transport, input) {
    const entry = exports.RAYDIUM_PROGRAM_IDS.farm;
    const owner = input.owner;
...
function executeResolvePrograms() {
    const programs = Object.entries(exports.RAYDIUM_PROGRAM_IDS).map(([key, val]) => ({
        id: val.id,
        name: val.name,
        key,
    }));
 ...
 * @since 1.1.0
 */
function createRaydiumOnchainTools(client, opts = {}) {
    const execute = createRaydiumOnchainExecutor(client.transport);
    return (0, shared_1.buildProtocolTools)(schemas_1.raydiumOnchainMethods, execute, {
...
Object.defineProperty(exports, "raydiumOnchainMethods", { enumerable: true, get: function () { return schemas_2.raydiumOnchainMethods; } });
Object.defineProperty(exports, "raydiumOnchainMethodNames", { enumerable: true, get: function () { return schemas_2.raydiumOnchainMethodNames; } });
