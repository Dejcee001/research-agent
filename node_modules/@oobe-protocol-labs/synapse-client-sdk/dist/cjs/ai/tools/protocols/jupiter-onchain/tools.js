...
exports.jupiterOnchainMethodNames = exports.jupiterOnchainMethods = exports.JUPITER_PROGRAM_IDS = void 0;
exports.createJupiterOnchainTools = createJupiterOnchainTools;
/**
...
const token_1 = require("../../../../accounts/token");
/* ═══════════════════════════════════════════════════════════════
...
const JUPITER_ID_REVERSE = new Map();
for (const [key, val] of Object.entries(exports.JUPITER_PROGRAM_IDS)) {
    JUPITER_ID_REVERSE.set(val.id, { key, name: val.name });
}
/* ═══════════════════════════════════════════════════════════════
 *  Execution dispatcher — on-chain via RPC transport
 * ═══════════════════════════════════════════════════════════════ */
function createJupiterOnchainExecutor(transport) {
...
async function executeSearchProgramAccounts(transport, input) {
    ...
    const entry = exports.JUPITER_PROGRAM_IDS[programKey];
    if (!entry)
        throw new Error(`Unknown Jupiter program: ${programKey}`);
    const cfg = {};
    if (input.encoding)
        cfg.encoding = input.encoding;
    else
 ...
 */
async function executeGetPositionsByOwner(transport, input, programKey, ownerOffset) {
    const entry = exports.JUPITER_PROGRAM_IDS[programKey];
    if (!entry)
        throw new Error(`Unknown Jupiter program: ${programKey}`);
...
async function executeGetPerpsPositions(transport, input) {
    const version = input.version || 'v2';
    const programKey = version === 'v1' ? 'perps_v1' : 'perps_v2';
    const entry = exports.JUPITER_PROGRAM_IDS[programKey];
    if (!entry)
        throw new Error(`Unknown Jupiter Perps version: ${version}`);
...
async function executeGetTokenMintInfo(transport, input) {
    const mint = input.mint;
    const commitment = input.commitment;
    const result = await (0, token_1.fetchMint)(transport, mint, { commitment });
    if (!result) {
...
async function executeGetTokenHoldings(transport, input) {
    const owner = input.owner;
    const mint = input.mint;
    const programId = input.programId || 'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA';
    // Build the filter
    const filter = mint
        ? { mint }
        : { programId };
...
function executeResolvePrograms() {
    const programs = Object.entries(exports.JUPITER_PROGRAM_IDS).map(([key, val]) => ({
        id: val.id,
        name: val.name,
        key,
    }));
 ...
 * @since 1.1.0
 */
function createJupiterOnchainTools(client, opts = {}) {
    const execute = createJupiterOnchainExecutor(client.transport);
    return (0, shared_1.buildProtocolTools)(schemas_1.jupiterOnchainMethods, execute, {
...
Object.defineProperty(exports, "jupiterOnchainMethods", { enumerable: true, get: function () { return schemas_2.jupiterOnchainMethods; } });
Object.defineProperty(exports, "jupiterOnchainMethodNames", { enumerable: true, get: function () { return schemas_2.jupiterOnchainMethodNames; } });
