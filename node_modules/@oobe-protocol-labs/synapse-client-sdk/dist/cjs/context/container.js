...
class SynapseContext {
     ...
     */
    constructor(scopeId = 'root', parent) {
    ...
    getRefRegistry() {
        if (!this._refRegistry) {
            this._refRegistry = new refs_1.RefRegistry({ enableLeakDetection: true });
        }
    ...
    bind(tokens) {
        this.assertNotDisposed();
        return (0, refs_1.createBinding)((t) => this.acquireRef(t), tokens);
    }
    /**
     ...
     * ```
     */
    enableMemoryGuard(config) {
        this.assertNotDisposed();
        if (this._memoryGuard)
            return this._memoryGuard;
        this._memoryGuard = new refs_1.MemoryGuard(this.getRefRegistry(), config);
        return this._memoryGuard;
    }
    /**
     ...
     * @since 1.2.0
     */
    register(token, provider, tags) {
        this.assertNotDisposed();
        const lifecycle = isValue(provider)
     ...
     * @since 1.2.0
     */
    registerIfMissing(token, provider, tags) {
        if (!this.has(token)) {
    ...
    resolve(token) {
        ...
        try {
            ...
            this.events.emit('error', { token, error: err });
            throw err;
        }
    }
    /**
    ...
    async dispose() {
        ...
        for (const entry of this.registry.values()) {
            if (entry.resolved && entry.instance != null && (0, types_1.isDisposable)(entry.instance)) {
                try {
    ...
    doResolve(token) {
        ...
        if (this.resolutionStack.has(token)) {
            ...
            throw new types_1.CircularDependencyError(chain);
        }
        // Find entry (local first, then parent)
        const entry = this.findEntry(token);
        if (!entry)
            throw new types_1.ServiceNotFoundError(token.name);
        ...
        if (isAsyncFactory(provider)) {
            throw new types_1.AsyncProviderError(token.name);
        }
    ...
    instantiate(provider) {
        ...
        if (isAlias(provider))
            return this.doResolve(provider.useExisting);
        // Should never reach here
        throw new errors_1.SynapseError(`Unknown provider type`);
    }
    ...
    findEntry(token) {
        ...
        if (parentEntry.lifecycle === 'scoped') {
            ...
            this.registry.set(token, localCopy);
            return localCopy;
        }
        return parentEntry;
    }
    ...
    assertNotDisposed() {
        if (this.disposed) {
            throw new errors_1.SynapseError('Context has been disposed and can no longer be used', -32103);
        }
    }
}
