...
exports.MemoryGuard = exports.RefRegistry = exports.WeakServiceRef = exports.ServiceRef = exports.RefReleasedError = void 0;
 ...
 * @since 1.2.0
 */
class RefReleasedError extends errors_1.SynapseError {
    constructor(tokenName) {
        super(`Service reference for "${tokenName}" has been released. ` +
            'Acquire a new ref with acquireRef() or check ref.alive before access.', -32104);
        this.name = 'RefReleasedError';
    }
}
exports.RefReleasedError = RefReleasedError;
// ── ServiceRef ─────────────────────────────────────────────────
/**
...
class ServiceRef {
     ...
     */
    constructor(_value, _token, _onRelease, 
    ...
    get alive() { return !this._released; }
    ...
    get ageMs() { return performance.now() - this._acquiredAt; }
    // ── Release ────────────────────────────────────────────────
    /**
    ...
    toWeak() {
        if (typeof this._value !== 'object' || this._value === null) {
            throw new errors_1.SynapseError(`Cannot create WeakRef for non-object service "${this._token.name}". ` +
                'WeakRef requires an object target.', -32105);
        }
        return new WeakServiceRef(this._value);
    }
}
...
exports.WeakServiceRef = WeakServiceRef;
// ── RefRegistry ────────────────────────────────────────────────
/**
...
class RefRegistry {
    ...
    acquire(token, value) {
        if (this._disposed) {
            throw new errors_1.SynapseError('RefRegistry has been disposed', -32106);
        }
        const id = this._nextId++;
        this._totalAcquired++;
        const ref = new ServiceRef(value, token, (r) => this._handleRelease(token, r), id);
        // Track in entries map
        let set = this._entries.get(token);
        if (!set) {
            set = new Set();
            this._entries.set(token, set);
        }
        set.add(ref);
        // Track active ID
        ...
        if (this._leakDetector) {
            this._leakDetector.register(ref, { tokenName: token.name, refId: id, acquiredAt: ref.acquiredAt }, ref);
        }
        return ref;
    }
    /** @internal Handle a ref being released. */
    _handleRelease(token, ref) {
...
class MemoryGuard {
    ...
    constructor(_registry, config = {}) {
        ...
        if (this._config.checkIntervalMs > 0) {
            this._timer = setInterval(() => this._periodicCheck(), this._config.checkIntervalMs);
    ...
    get stats() { return this._registry.stats; }
 ...
 * ```
 */
function createBinding(acquireFn, tokens) {
    const refs = new Map();
    let released = false;
    const services = new Proxy({}, {
        get(_target, prop) {
            if (typeof prop === 'symbol')
                return undefined;
            if (released) {
                throw new errors_1.SynapseError('ServiceBinding has been released. Cannot access services after release.', -32107);
            }
