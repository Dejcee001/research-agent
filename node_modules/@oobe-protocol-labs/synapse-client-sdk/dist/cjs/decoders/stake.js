...
exports.decodeStakeAccount = decodeStakeAccount;
...
exports.STAKE_PROGRAM_ID = 'Stake11111111111111111111111111111111111111';
// ── Decoder ────────────────────────────────────────────────────
/**
...
function decodeStakeAccount(data) {
    ...
    if (data.length < 124) {
        throw new Error(`Stake account data too short for meta: expected >= 124 bytes, got ${data.length}`);
    }
    ...
    if (data.length < 200) {
        throw new Error(`Stake account data too short for delegation: expected >= 200 bytes, got ${data.length}`);
    }
