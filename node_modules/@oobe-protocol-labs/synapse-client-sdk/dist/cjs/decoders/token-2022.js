"use strict";
/**
 * SPL Token-2022 (Token Extensions) decoder.
 *
 * Parses TLV (Type-Length-Value) extensions appended after the standard
 * 165-byte token account or 82-byte mint layout.
 *
 * @module decoders/token-2022
 * @since 1.1.0
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.TOKEN_2022_PROGRAM_ID = exports.TokenExtensionType = void 0;
exports.decodeToken2022Account = decodeToken2022Account;
exports.decodeToken2022Mint = decodeToken2022Mint;
const layout_1 = require("./layout");
const token_1 = require("./token");
// ── Extension type enum ────────────────────────────────────────
/** Known Token-2022 extension types. @since 1.1.0 */
var TokenExtensionType;
(function (TokenExtensionType) {
    TokenExtensionType[TokenExtensionType["Uninitialized"] = 0] = "Uninitialized";
    TokenExtensionType[TokenExtensionType["TransferFeeConfig"] = 1] = "TransferFeeConfig";
    TokenExtensionType[TokenExtensionType["TransferFeeAmount"] = 2] = "TransferFeeAmount";
    TokenExtensionType[TokenExtensionType["MintCloseAuthority"] = 3] = "MintCloseAuthority";
    TokenExtensionType[TokenExtensionType["ConfidentialTransferMint"] = 4] = "ConfidentialTransferMint";
    TokenExtensionType[TokenExtensionType["ConfidentialTransferAccount"] = 5] = "ConfidentialTransferAccount";
    TokenExtensionType[TokenExtensionType["DefaultAccountState"] = 6] = "DefaultAccountState";
    TokenExtensionType[TokenExtensionType["ImmutableOwner"] = 7] = "ImmutableOwner";
    TokenExtensionType[TokenExtensionType["MemoTransfer"] = 8] = "MemoTransfer";
    TokenExtensionType[TokenExtensionType["NonTransferable"] = 9] = "NonTransferable";
    TokenExtensionType[TokenExtensionType["InterestBearingConfig"] = 10] = "InterestBearingConfig";
    TokenExtensionType[TokenExtensionType["CpiGuard"] = 11] = "CpiGuard";
    TokenExtensionType[TokenExtensionType["PermanentDelegate"] = 12] = "PermanentDelegate";
    TokenExtensionType[TokenExtensionType["NonTransferableAccount"] = 13] = "NonTransferableAccount";
    TokenExtensionType[TokenExtensionType["TransferHook"] = 14] = "TransferHook";
    TokenExtensionType[TokenExtensionType["TransferHookAccount"] = 15] = "TransferHookAccount";
    TokenExtensionType[TokenExtensionType["MetadataPointer"] = 18] = "MetadataPointer";
    TokenExtensionType[TokenExtensionType["TokenMetadata"] = 19] = "TokenMetadata";
    TokenExtensionType[TokenExtensionType["GroupPointer"] = 20] = "GroupPointer";
    TokenExtensionType[TokenExtensionType["GroupMemberPointer"] = 22] = "GroupMemberPointer";
})(TokenExtensionType || (exports.TokenExtensionType = TokenExtensionType = {}));
/** SPL Token-2022 Program ID. @since 1.1.0 */
exports.TOKEN_2022_PROGRAM_ID = 'TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb';
// ── TLV parser ─────────────────────────────────────────────────
/**
 * Parse TLV extensions from raw account data.
 *
 * Layout after base account:
 * - 1 byte: AccountType discriminator (1 = Mint, 2 = Account)
 * - TLV entries: [u16 type, u16 length, ...data] repeated
 *
 * @internal
 */
function parseTlvExtensions(data, baseSize) {
    const extensions = [];
    // Skip base layout + 1-byte AccountType discriminator
    let offset = baseSize + 1;
    while (offset + 4 <= data.length) {
        const extType = data[offset] | (data[offset + 1] << 8); // u16 LE
        const extLen = data[offset + 2] | (data[offset + 3] << 8); // u16 LE
        offset += 4;
        // End marker: type=0 && length=0
        if (extType === 0 && extLen === 0)
            break;
        if (offset + extLen > data.length)
            break;
        const extData = data.slice(offset, offset + extLen);
        extensions.push(decodeExtension(extType, extData));
        offset += extLen;
    }
    return extensions;
}
/** @internal */
function decodeExtension(extType, data) {
    const r = new layout_1.AccountReader(data);
    switch (extType) {
        case TokenExtensionType.TransferFeeConfig: {
            const transferFeeConfigAuthority = r.pubkey();
            const withdrawWithheldAuthority = r.pubkey();
            const withheldAmount = r.u64();
            const olderEpoch = r.u64();
            const olderMax = r.u64();
            const olderBps = r.u16();
            const newerEpoch = r.u64();
            const newerMax = r.u64();
            const newerBps = r.u16();
            return {
                type: 'TransferFeeConfig',
                transferFeeConfigAuthority,
                withdrawWithheldAuthority,
                withheldAmount,
                olderTransferFee: { epoch: olderEpoch, maximumFee: olderMax, transferFeeBasisPoints: olderBps },
                newerTransferFee: { epoch: newerEpoch, maximumFee: newerMax, transferFeeBasisPoints: newerBps },
            };
        }
        case TokenExtensionType.TransferFeeAmount:
            return { type: 'TransferFeeAmount', withheldAmount: r.u64() };
        case TokenExtensionType.MintCloseAuthority:
            return { type: 'MintCloseAuthority', closeAuthority: r.pubkey() };
        case TokenExtensionType.DefaultAccountState:
            return { type: 'DefaultAccountState', state: r.u8() };
        case TokenExtensionType.ImmutableOwner:
            return { type: 'ImmutableOwner' };
        case TokenExtensionType.MemoTransfer:
            return { type: 'MemoTransfer', requireIncomingTransferMemos: r.bool() };
        case TokenExtensionType.NonTransferable:
            return { type: 'NonTransferable' };
        case TokenExtensionType.InterestBearingConfig: {
            const rateAuthority = r.pubkey();
            const initializationTimestamp = r.i64();
            const preUpdateAverageRate = r.u16();
            const lastUpdateTimestamp = r.i64();
            const currentRate = r.u16();
            return {
                type: 'InterestBearingConfig',
                rateAuthority,
                initializationTimestamp,
                preUpdateAverageRate,
                lastUpdateTimestamp,
                currentRate,
            };
        }
        case TokenExtensionType.PermanentDelegate:
            return { type: 'PermanentDelegate', delegate: r.pubkey() };
        case TokenExtensionType.MetadataPointer:
            return { type: 'MetadataPointer', authority: r.pubkey(), metadataAddress: r.pubkey() };
        default:
            return { type: 'Unknown', extensionType: extType, data };
    }
}
// ── Public decoders ────────────────────────────────────────────
/**
 * Decode a Token-2022 token account (base layout + TLV extensions).
 *
 * @param data - Raw account data (>= 165 bytes).
 * @returns Decoded account with parsed extensions.
 * @since 1.1.0
 */
function decodeToken2022Account(data) {
    const base = (0, token_1.decodeTokenAccount)(data);
    const extensions = data.length > token_1.TOKEN_ACCOUNT_SIZE + 1
        ? parseTlvExtensions(data, token_1.TOKEN_ACCOUNT_SIZE)
        : [];
    return { ...base, extensions };
}
/**
 * Decode a Token-2022 mint (base layout + TLV extensions).
 *
 * @param data - Raw account data (>= 82 bytes).
 * @returns Decoded mint with parsed extensions.
 * @since 1.1.0
 */
function decodeToken2022Mint(data) {
    const base = (0, token_1.decodeMint)(data);
    const extensions = data.length > token_1.MINT_SIZE + 1
        ? parseTlvExtensions(data, token_1.MINT_SIZE)
        : [];
    return { ...base, extensions };
}
//# sourceMappingURL=token-2022.js.map