...
class HttpTransport {
    ...
    constructor(cfg) {
        ...
        this.headers = {
            ...
            ...(!(0, env_1.isBrowser)() ? { 'User-Agent': env_1.SDK_USER_AGENT } : {}),
            ...(cfg.headers ?? {})
        };
    }
    /**
    ...
    async request(method, params = [], opts = {}) {
        ...
        if (lastErr?.error)
            this.throwRpcError(lastErr.error, method);
        throw new errors_1.SynapseError('Unknown RPC error', -32000);
    }
    /**
    ...
    async batch(requests, opts = {}) {
        ...
        try {
            ...
            if (!res.ok)
                throw new errors_1.NetworkError(`HTTP ${res.status}: ${res.statusText}`);
            const body = await res.json();
            return body.map(r => {
                if (r.error)
                    this.throwRpcError(r.error, 'batch');
                return r.result;
            });
        }
        catch (e) {
            if (e instanceof errors_1.SynapseError)
                throw e;
            if (e.name === 'AbortError')
                throw new errors_1.TimeoutError(timeout);
            throw new errors_1.NetworkError(`Batch failed: ${e.message}`, e);
        }
        finally {
            clearTimeout(timer);
        }
    ...
    async send(req, timeoutMs) {
        ...
        try {
            ...
            if (e.name === 'AbortError')
                throw new errors_1.TimeoutError(timeout);
            throw new errors_1.NetworkError(`Request failed: ${e.message}`, e);
        }
        finally {
            clearTimeout(timer);
        }
    }
    throwRpcError(err, method) {
        const data = err.data;
        if (data?.upstreamName || data?.upstreamUrl) {
            throw new errors_1.UpstreamError(err.message, err.code, data.upstreamName, data.upstreamUrl);
        }
        throw new errors_1.SynapseError(`[${method}] ${err.message}`, err.code, err.data);
    }
}
