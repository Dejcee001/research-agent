...
class SynapseClient {
    ...
    constructor(config) {
        ...
        if (config.debug)
            console.log('[Synapse SDK] initialized:', config.endpoint, `(${(0, env_1.isBrowser)() ? 'browser' : 'server'})`);
    }
    /**
     ...
     * ```
     */
    static fromEndpoint(config) {
        const endpoint = (0, synapse_1.resolveEndpoint)(config.network, config.region);
        const clientCfg = (0, synapse_1.toClientConfig)(endpoint, {
     ...
     */
    batch(requests, opts = {}) {
        return this.transport.batch(requests, opts);
    }
    ...
    get rpc() {
        if (!this._rpc) {
            const { SolanaRpc: Ctor } = require('../rpc/solana-rpc');
    ...
    get das() {
        if (!this._das) {
            const { DasClient: Ctor } = require('../das/client');
    ...
    get ws() {
        if (!this._ws) {
            const { WsClient: Ctor } = require('../websocket/client');
    ...
    get grpc() {
        if ((0, env_1.isBrowser)()) {
            throw new Error('GrpcTransport is not available in browser environments. ' +
                'gRPC requires native Node.js bindings (@grpc/grpc-js). ' +
                'Use client.rpc or client.ws for browser/Next.js client components.');
        }
        if (!this._grpc) {
            const { GrpcTransport: Ctor } = require('../grpc/transport');
