 ...
 *  1. Main / Info — /main/*
 * ═══════════════════════════════════════════════════════════════ */
register('getInfo', z.object({}), z.object({
...
}), 'Get Raydium protocol-wide stats: TVL and 24-hour trading volume.', { httpMethod: 'GET', path: '/main/info' });
register('getChainTime', z.object({}), z.object({
...
}), 'Get the current Solana chain time from Raydium nodes.', { httpMethod: 'GET', path: '/main/chain-time' });
register('getClmmConfig', z.object({}), z.array(z.object({
    id: z.string(),
    index: z.number(),
...
})), 'Get configuration for all CLMM (Concentrated Liquidity) pool tiers, including fee rates and tick spacing.', { httpMethod: 'GET', path: '/main/clmm-config' });
register('getCpmmConfig', z.object({}), z.array(z.object({
    id: z.string(),
    index: z.number(),
    protocolFeeRate: z.number(),
    tradeFeeRate: z.number(),
...
})), 'Get configuration for all CPMM (Constant Product) pool tiers, including fee rates and pool creation costs.', { httpMethod: 'GET', path: '/main/cpmm-config' });
register('getAutoFee', z.object({}), z.object({
    fee: z.number().describe('Recommended transaction priority fee (lamports)'),
    feeLevel: z.string().optional().describe('Fee urgency level'),
}), 'Get the recommended auto-computed transaction priority fee.', { httpMethod: 'GET', path: '/main/auto-fee' });
/* ═══════════════════════════════════════════════════════════════
 *  2. Mint / Token — /mint/*
 * ═══════════════════════════════════════════════════════════════ */
register('getMintList', z.object({}), z.array(z.object({
    address: zMint,
...
})), 'Get the Raydium default token mint list with metadata.', { httpMethod: 'GET', path: '/mint/list' });
register('getMintInfo', z.object({
...
})), 'Get detailed token mint info for one or more mints.', { httpMethod: 'GET', path: '/mint/ids' });
register('getMintPrice', z.object({
    mints: z.array(zMint).min(1).max(100).describe('Token mint addresses to price'),
}), z.record(zMint, z.number()), 'Get USD prices for one or more token mints.', { httpMethod: 'GET', path: '/mint/price' });
/* ═══════════════════════════════════════════════════════════════
 *  3. Pools — /pools/*
 * ═══════════════════════════════════════════════════════════════ */
...
register('getPoolInfo', z.object({
    ids: z.array(zPoolId).min(1).max(100).describe('Pool IDs to query'),
}), z.array(zPoolInfo), 'Get detailed pool info (TVL, price, APR, volume) for specific Raydium pool IDs.', { httpMethod: 'GET', path: '/pools/info/ids' });
register('getPoolsByLpMint', z.object({
    lpMints: z.array(zMint).min(1).max(100).describe('LP mint addresses'),
}), z.array(zPoolInfo), 'Find Raydium pools by their LP token mint address.', { httpMethod: 'GET', path: '/pools/info/lps' });
register('getPoolList', z.object({
...
}), 'Get a paginated, sortable list of all Raydium pools with full stats.', { httpMethod: 'GET', path: '/pools/info/list' });
register('getPoolsByTokenMint', z.object({
    ...
    data: z.array(zPoolInfo),
    hasNextPage: z.boolean(),
}), 'Find Raydium pools by token mint(s). Great for discovering where to swap a specific token.', { httpMethod: 'GET', path: '/pools/info/mint' });
register('getPoolLiquidity', z.object({
    poolId: zPoolId,
...
}), 'Get historical liquidity data for a pool (useful for charting).', { httpMethod: 'GET', path: '/pools/line/liquidity' });
register('getClmmPosition', z.object({
    poolId: zPoolId,
}), z.object({
    poolId: zPoolId,
    data: z.array(z.object({
...
}), 'Get current CLMM concentrated liquidity position distribution for a pool.', { httpMethod: 'GET', path: '/pools/line/position' });
/* ═══════════════════════════════════════════════════════════════
 *  4. Farms — /farms/*
 * ═══════════════════════════════════════════════════════════════ */
...
register('getFarmInfo', z.object({
    ids: z.array(z.string()).min(1).max(100).describe('Farm IDs to query'),
}), z.array(zFarmInfo), 'Get detailed farm info (APR, rewards, TVL) for specific Raydium farm IDs.', { httpMethod: 'GET', path: '/farms/info/ids' });
register('getFarmByLpMint', z.object({
    lpMints: z.array(zMint).min(1).max(100).describe('LP mint addresses to search farms for'),
}), z.array(zFarmInfo), 'Find Raydium yield farms by the LP mint address they accept.', { httpMethod: 'GET', path: '/farms/info/lp' });
/* ═══════════════════════════════════════════════════════════════
 *  Export
 * ═══════════════════════════════════════════════════════════════ */
/**
 ...
 * @since 1.0.0
 */
export const raydiumMethodNames = methods.map((m) => m.name);
