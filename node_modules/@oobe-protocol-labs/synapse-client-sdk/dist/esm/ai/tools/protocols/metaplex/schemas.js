 ...
 *  1. DAS Core Methods
 * ═══════════════════════════════════════════════════════════════ */
register('getAsset', z.object({
    id: zPubkey.describe('Asset ID (mint address or asset ID)'),
    commitment: zCommitment,
}), zDasAsset, 'Get complete metadata, ownership, compression, and royalty info for a single digital asset (NFT, cNFT, Core, or fungible).');
register('getAssets', z.object({
    ids: z.array(zPubkey).min(1).max(1000).describe('Asset IDs to fetch'),
    commitment: zCommitment,
}), z.array(zDasAsset), 'Batch-fetch metadata for multiple digital assets in a single RPC call.');
register('getAssetProof', z.object({
    id: zPubkey.describe('Compressed NFT asset ID'),
    commitment: zCommitment,
}), zDasProof, 'Get the Merkle tree proof for a compressed NFT. Required for transfers and burns of cNFTs.');
register('getAssetProofs', z.object({
    ids: z.array(zPubkey).min(1).max(1000).describe('Compressed NFT asset IDs'),
    commitment: zCommitment,
}), z.record(zPubkey, zDasProof), 'Batch-fetch Merkle tree proofs for multiple compressed NFTs.');
register('getAssetsByOwner', z.object({
    ownerAddress: zPubkey.describe('Wallet address of the owner'),
    ...zPagination,
    sortBy: zSortBy,
    ...zDisplayOpts,
    commitment: zCommitment,
}), zDasPage, 'Get all digital assets (NFTs, cNFTs, fungibles) owned by a wallet address.');
register('getAssetsByCreator', z.object({
    ...
    ...zDisplayOpts,
    commitment: zCommitment,
}), zDasPage, 'Get all digital assets created by a specific creator address.');
register('getAssetsByCollection', z.object({
    ...
    ...zDisplayOpts,
    commitment: zCommitment,
}), zDasPage, 'Get all assets in a Metaplex collection. Uses DAS getAssetsByGroup with groupKey="collection".');
register('getAssetsByAuthority', z.object({
    authorityAddress: zPubkey.describe('Authority address'),
    ...zPagination,
    sortBy: zSortBy,
    commitment: zCommitment,
}), zDasPage, 'Get all digital assets managed by a specific authority address.');
register('searchAssets', z.object({
    ...
    ...zDisplayOpts,
    commitment: zCommitment,
}), zDasPage, 'Advanced multi-filter search across all indexed digital assets (NFTs, cNFTs, fungibles, Core assets).');
register('getAssetSignatures', z.object({
...
register('getTokenAccounts', z.object({
 ...
 * ═══════════════════════════════════════════════════════════════ */
register('resolveCollection', z.object({
 ...
 * @since 1.0.0
 */
export const metaplexMethodNames = methods.map((m) => m.name);
