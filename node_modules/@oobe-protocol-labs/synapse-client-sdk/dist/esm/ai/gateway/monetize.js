...
export class MonetizeError extends Error {
     ...
     */
    constructor(message, code = 'MONETIZE_ERROR') {
        super(message);
        this.code = code;
 ...
 * @since 1.3.0
 */
export async function createMonetizedTools(gateway, sessionId, toolkit, config = {}) {
    ...
    const { autoPublish = true, descriptionPrefix = '[Monetized]', includeAttestation, prefix = 'monetized_', region, commitments = ['confirmed'], tiers, } = config;
    /* ── Mutable metrics ── */
    const metrics = {
        totalCalls: 0,
        totalRevenue: 0n,
    ...
    for (const method of toolkit.methods) {
        ...
        const wrappedTool = lcTool(async (input) => {
            if (_disposed) {
            ...
            try {
                ...
                const rawResult = await originalTool.invoke(input);
                ...
                const attested = await gateway.validator.wrapResult(parsed, sessionId, `${toolkit.protocol}:${method.name}`, input, 0, // slot — protocol calls are off-chain
                latencyMs, snap.callsMade + 1, shouldAttest);
 ...
 * @since 1.3.0
 */
export async function createMultiProtocolMonetizedTools(gateway, sessionId, toolkits, config = {}) {
    const entries = Object.entries(toolkits);
    const monetized = await Promise.all(entries.map(([, tk]) => createMonetizedTools(gateway, sessionId, tk, config)));
    const result = {};
    const allTools = [];
