...
export class ToolMarketplace {
     ...
     * @since 1.0.0
     */
    listBundles(sellerId) {
        const all = [...this.bundles.values()];
        if (sellerId)
            return all.filter(b => b.seller.id === sellerId);
        return all;
    }
    /* ── Search & Discovery ──────────────────────────────────── */
    /**
    ...
    search(query = {}) {
        ...
        if (query.tags?.length) {
            results = results.filter(l => query.tags.some(tag => l.seller.tags?.includes(tag)));
        }
        // Sort
        if (query.sortBy) {
     ...
     * @since 1.0.0
     */
    reportAttestation(sellerId, verified, latencyMs) {
    ...
    computeReputationScore(data) {
        ...
        return Math.round((verificationRate * 400) + (volumeScore * 300) + (latencyScore * 300));
    }
