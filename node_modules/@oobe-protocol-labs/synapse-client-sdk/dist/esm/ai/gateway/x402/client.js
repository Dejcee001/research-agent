 ...
 * @since 1.0.0
 */
import { X402_HEADER_PAYMENT_REQUIRED, X402_HEADER_PAYMENT_SIGNATURE, X402_HEADER_PAYMENT_RESPONSE, X402_STATUS_CODE, } from './types.js';
/* ═══════════════════════════════════════════════════════════════
...
function decodePaymentHeader(base64) {
...
export class NoAcceptablePaymentError extends X402ClientError {
     ...
     * @since 1.0.0
     */
    constructor(paymentRequired, reason) {
...
export class PaymentSigningError extends X402ClientError {
     ...
     * @since 1.0.0
     */
    constructor(requirements, cause) {
 ...
 * @since 1.0.0
 */
export function defaultRequirementsSelector(accepts, config) {
...
export class X402Client {
    ...
    async fetch(url, init) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            ...
            retryHeaders.set(X402_HEADER_PAYMENT_SIGNATURE, encodePaymentHeader(paymentResult.paymentPayload));
            ...
            if (paymentResponseRaw) {
                try {
                    settlement = decodePaymentHeader(paymentResponseRaw);
                }
                catch {
     ...
     * @since 1.0.0
     */
    async interceptResponse(statusCode, headers, body) {
        ...
        const paymentRequired = decodePaymentHeader(paymentRequiredRaw);
        // Select requirements
     ...
     * @since 1.0.0
     */
    parseSettlementResponse(headers) {
    ...
    /* ── Internal helpers ────────────────────────────────────── */
    async handlePaymentRequired(response, _url) {
        ...
        if (!paymentRequiredRaw) {
            ...
            try {
                ...
                if (body?.accepts?.length > 0) {
                    return this.selectAndSign(body);
                }
            }
            catch {
    ...
    async selectAndSign(paymentRequired) {
        // Select requirements
        const requirements = this.selector(paymentRequired.accepts, this.config);
        if (!requirements) {
            return null;
