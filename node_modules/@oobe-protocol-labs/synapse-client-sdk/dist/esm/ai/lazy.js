 ...
 */
export async function getJupiterTools(config = {}) {
    const hash = JSON.stringify(config);
    if (_jupiterCache && _jupiterConfigHash === hash)
        return _jupiterCache;
    const { createJupiterTools } = await import('./tools/protocols/jupiter/index.js');
    _jupiterCache = createJupiterTools(config);
    _jupiterConfigHash = hash;
    return _jupiterCache;
}
/* ── Raydium ────────────────────────────────────────────────── */
let _raydiumCache = null;
 ...
 */
export async function getRaydiumTools(config = {}) {
    const hash = JSON.stringify(config);
    if (_raydiumCache && _raydiumConfigHash === hash)
        return _raydiumCache;
    const { createRaydiumTools } = await import('./tools/protocols/raydium/index.js');
    _raydiumCache = createRaydiumTools(config);
