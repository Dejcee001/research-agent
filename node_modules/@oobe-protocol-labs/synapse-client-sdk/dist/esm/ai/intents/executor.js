 ...
 * @since 1.3.0
 */
import { IntentError, UnresolvedReferenceError } from './types.js';
/* ═══════════════════════════════════════════════════════════════
 *  Reference Resolver
 * ═══════════════════════════════════════════════════════════════ */
/**
    ...
    for (const [key, value] of Object.entries(params)) {
        if (typeof value === 'string' && value.startsWith('$')) {
            ...
            if (stepResult.status !== 'completed' && stepResult.status !== 'simulated') {
                throw new UnresolvedReferenceError(`${value} (step '${refStepId}' has status '${stepResult.status}')`, stepId);
            }
            let val = stepResult.output;
            for (const field of fieldPath) {
                if (val === null || val === undefined) {
                    throw new UnresolvedReferenceError(`${value} — field '${field}' not found (value is ${val})`, stepId);
                }
                if (typeof val === 'object') {
                    val = val[field];
                }
                else {
                    throw new UnresolvedReferenceError(`${value} — cannot traverse '${field}' on non-object`, stepId);
                }
            }
            resolved[key] = val;
        }
...
export class IntentExecutor {
    ...
    async executeStep(step, results, simulate) {
        ...
        try {
            ...
            if (!protocolTools) {
                throw new IntentError(`No tools registered for protocol '${step.protocol}'`, 'PROTOCOL_NOT_FOUND', step.id);
            }
            const tool = protocolTools[step.method];
            if (!tool) {
                throw new IntentError(`Method '${step.method}' not found in protocol '${step.protocol}'`, 'METHOD_NOT_FOUND', step.id);
            }
            // Execute with timeout
            const timeoutMs = step.timeoutMs ?? this.defaultTimeout;
            const rawOutput = await Promise.race([
                tool.invoke(resolvedParams),
                new Promise((_, reject) => setTimeout(() => reject(new IntentError(`Step '${step.id}' timed out after ${timeoutMs}ms`, 'TIMEOUT', step.id)), timeoutMs)),
            ]);
            // Parse output
            let output;
            try {
