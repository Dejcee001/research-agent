...
regMpl('deployCollection', z.object({
...
regMpl('updateMetadata', z.object({
    mint: zMint.describe('NFT mint address'),
...
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Update NFT metadata fields (name, symbol, URI, creators, royalties, authority).');
regMpl('verifyCreator', z.object({
    mint: zMint,
    creator: zPubkey.describe('Creator wallet to verify'),
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Verify a creator on an NFT\'s metadata (requires creator\'s signature).');
regMpl('verifyCollection', z.object({
...
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Verify an NFT as part of a collection.');
regMpl('setAndVerifyCollection', z.object({
    mint: zMint,
    collection: zMint,
    collectionAuthority: zPubkey,
    updateAuthority: zPubkey.describe('NFT update authority'),
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Set and verify a collection on an NFT in a single transaction.');
regMpl('delegateAuthority', z.object({
    mint: zMint,
...
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Delegate a specific authority (update, mint, freeze, collection) to another wallet.');
regMpl('revokeAuthority', z.object({
    mint: zMint,
    authority: zPubkey.describe('Current authority holder'),
    authorityType: z.enum(['update', 'mint', 'freeze', 'collection']),
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Revoke (remove) a specific authority. Irreversible for some types.');
regMpl('configureRoyalties', z.object({
    mint: zMint,
...
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Configure royalty fee and creator splits for an NFT.');
/* ═══════════════════════════════════════════════════════════════
 *  2. 3.Land NFT Marketplace
 * ═══════════════════════════════════════════════════════════════ */
const { register: reg3land, methods: threeLandMethods } = createMethodRegistry('3land');
reg3land('createCollection', z.object({
...
reg3land('cancelListing', z.object({
    seller: zPubkey,
    listingId: z.string().describe('3.Land listing ID to cancel'),
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Cancel an active NFT listing on 3.Land.');
reg3land('buyNFT', z.object({
    buyer: zPubkey,
...
regDas('getAssetsByOwner', z.object({
...
regDas('getAssetsByCreator', z.object({
...
regDas('getAssetsByCollection', z.object({
...
export const allNftMethods = [...metaplexNftMethods, ...threeLandMethods, ...dasMethods];
