 ...
 */
import { z } from 'zod';
import { createMethodRegistry } from '../../tools/protocols/shared.js';
...
regRayPool('createCPMM', z.object({
    creator: zPubkey,
...
regRayPool('createCLMM', z.object({
    creator: zPubkey,
    mintA: zMint,
    mintB: zMint,
...
regRayPool('createAMMv4', z.object({
    creator: zPubkey,
...
regRayPool('addLiquidity', z.object({
    wallet: zPubkey,
...
regRayPool('removeLiquidity', z.object({
    wallet: zPubkey,
    poolId: zPubkey,
...
regOrca('closePosition', z.object({
    wallet: zPubkey,
    positionMint: zMint,
}), z.object({
    tx: zTx,
...
regManifest('createMarket', z.object({
    creator: zPubkey,
...
regManifest('placeLimitOrder', z.object({
    wallet: zPubkey,
    marketId: zPubkey,
...
regManifest('cancelOrder', z.object({
    wallet: zPubkey,
    marketId: zPubkey,
    orderId: z.string(),
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Cancel a limit order on a Manifest market.');
regManifest('getOrderbook', z.object({
    marketId: zPubkey,
...
regMeteora('createDynamicPool', z.object({
    creator: zPubkey,
    mintA: zMint,
    mintB: zMint,
    amountA: zAmount,
...
regMeteora('createDLMMPool', z.object({
    creator: zPubkey,
...
regMeteora('addDLMMLiquidity', z.object({
    wallet: zPubkey,
    poolAddress: zPubkey,
    amountX: zAmount,
    amountY: zAmount,
...
regMeteora('removeDLMMLiquidity', z.object({
    wallet: zPubkey,
    positionAddress: zPubkey,
...
regMeteora('createAlphaVault', z.object({
    creator: zPubkey,
    poolAddress: zPubkey,
...
regOb('cancelOrder', z.object({
    wallet: zPubkey,
    marketId: zPubkey,
    orderId: z.string(),
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Cancel an open order on an Openbook market.');
/* ═══════════════════════════════════════════════════════════════
 *  7. Drift — Vaults, Perps, Lending, Borrowing
 * ═══════════════════════════════════════════════════════════════ */
const { register: regDrift, methods: driftMethods } = createMethodRegistry('drift');
regDrift('deposit', z.object({
    wallet: zPubkey,
    mint: zMint.describe('Token to deposit (e.g. USDC, SOL)'),
    amount: zAmount,
    subAccountId: z.number().int().optional().default(0),
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Deposit tokens into a Drift account for trading or lending.');
regDrift('withdraw', z.object({
    wallet: zPubkey,
    mint: zMint,
    amount: zAmount,
    subAccountId: z.number().int().optional().default(0),
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Withdraw tokens from a Drift account.');
regDrift('openPerpPosition', z.object({
    wallet: zPubkey,
...
regDrift('closePerpPosition', z.object({
    wallet: zPubkey,
...
regDrift('getPositions', z.object({
    wallet: zPubkey,
    subAccountId: z.number().int().optional().default(0),
}), z.object({
...
regAdrena('openPosition', z.object({
    wallet: zPubkey,
...
regAdrena('closePosition', z.object({
    wallet: zPubkey,
    positionKey: zPubkey,
}), z.object({
    tx: zTx,
...
regAdrena('addCollateral', z.object({
    wallet: zPubkey,
    positionKey: zPubkey,
    amount: zAmount,
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Add collateral to an existing Adrena position.');
regAdrena('removeCollateral', z.object({
    wallet: zPubkey,
    positionKey: zPubkey,
    amount: zAmount,
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Remove collateral from an Adrena position.');
regAdrena('getPositions', z.object({
    wallet: zPubkey,
}), z.object({
...
regJito('getBundleStatus', z.object({
    bundleId: z.string(),
}), z.object({
...
regJito('getTipEstimate', z.object({}), z.object({
 ...
 *  Exports
 * ═══════════════════════════════════════════════════════════════ */
export { pumpMethods, raydiumPoolMethods, orcaMethods, manifestMethods, meteoraMethods, openbookMethods, driftMethods, adrenaMethods, luloMethods, jitoMethods, };
