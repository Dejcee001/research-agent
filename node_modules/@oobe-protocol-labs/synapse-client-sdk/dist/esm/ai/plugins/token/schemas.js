...
regSpl('getTokenAccounts', z.object({
...
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Mint additional tokens to a destination wallet (requires mint authority).');
regSpl('burn', z.object({
    mint: zMint,
    owner: zPubkey.describe('Token account owner'),
    amount: zAmount.describe('Amount to burn'),
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Burn SPL tokens from the owner\'s token account.');
regSpl('freezeAccount', z.object({
    mint: zMint,
    account: zPubkey.describe('Token account to freeze'),
    freezeAuthority: zPubkey.describe('Freeze authority wallet'),
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Freeze a token account (prevents all transfers). Requires freeze authority.');
regSpl('thawAccount', z.object({
    mint: zMint,
    account: zPubkey.describe('Token account to thaw'),
    freezeAuthority: zPubkey.describe('Freeze authority wallet'),
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Thaw (unfreeze) a previously frozen token account.');
regSpl('closeAccount', z.object({
...
}), z.object({ tx: zTx, reclaimedLamports: zAmount, signature: z.string().optional() }), 'Close a token account and reclaim the rent SOL.');
regSpl('rugCheck', z.object({
...
regStake('unstakeSOL', z.object({
    wallet: zPubkey,
    stakeAccount: zPubkey.describe('Stake account to deactivate and withdraw'),
}), z.object({ tx: zTx, signature: z.string().optional() }), 'Deactivate and withdraw SOL from a stake account.');
regStake('getStakeAccounts', z.object({
...
regStake('unstakeJupSOL', z.object({
    wallet: zPubkey,
...
regStake('stakeSolayer', z.object({
    wallet: zPubkey,
...
regStake('unstakeSolayer', z.object({
    wallet: zPubkey,
...
regBridge('bridgeWormhole', z.object({
...
regBridge('bridgeWormholeStatus', z.object({
...
regBridge('bridgeDeBridge', z.object({
...
regBridge('bridgeDeBridgeStatus', z.object({
    orderId: z.string().describe('deBridge DLN order ID'),
}), z.object({
...
export const allTokenMethods = [...splMethods, ...stakingMethods, ...bridgingMethods];
