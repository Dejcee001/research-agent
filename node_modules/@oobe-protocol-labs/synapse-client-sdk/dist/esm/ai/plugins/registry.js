 ...
 * @since 2.0.0
 */
import { buildProtocolTools } from '../tools/protocols/shared.js';
/* ═══════════════════════════════════════════════════════════════
 *  SynapseAgentKit
 * ═══════════════════════════════════════════════════════════════ */
/**
...
export class SynapseAgentKit {
    ...
    use(plugin, config) {
        ...
        if (plugin.meta.dependencies) {
            for (const dep of plugin.meta.dependencies) {
                if (!this._plugins.has(dep)) {
                    throw new Error(`Plugin "${plugin.meta.id}" requires "${dep}" — install it first with .use()`);
                }
            }
        }
        // Install the plugin
        ...
        for (const protocol of plugin.protocols) {
            const toolkit = buildProtocolTools(protocol.methods, async (method, input) => installResult.executor(method, input, context), {
     ...
     * @since 2.0.0
     */
    getProtocolToolkit(pluginId, protocolId) {
