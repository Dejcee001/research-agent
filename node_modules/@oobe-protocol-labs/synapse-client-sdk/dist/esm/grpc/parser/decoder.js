 ...
 * @since 1.0.0
 */
import { Pubkey, Sig, Slot, Lamports } from '../../core/types.js';
import { lamportsToSol } from '../../utils/helpers.js';
import { resolveProgram } from './programs.js';
/* ═══════════════════════════════════════════════════════════════
...
export function rawBufferToBase64(buf) {
    const bytes = rawBufferToBytes(buf);
 ...
 * @since 1.0.0
 */
export function computeBalanceChanges(preBalances, postBalances, skipZero = true) {
    const changes = [];
 ...
 * @since 1.0.0
 */
export function computeTokenBalanceChanges(preTokenBalances, postTokenBalances) {
 ...
 */
function decodeMessageAccountKeys(messageBytes) {
    const accountKeys = [];
    const instructions = [];
    let recentBlockhash = '';
    try {
 ...
 * @since 1.0.0
 */
export function parseTransaction(raw, config = {}, timestamp) {
    const { skipZeroBalanceChanges = true, customPrograms, includeInnerInstructions = true, includeTokenBalances = true, } = config;
    ...
    const { accountKeys: decodedKeys, instructions: decodedIx, recentBlockhash } = decodeMessageAccountKeys(messageBytes);
    const accountKeys = decodedKeys.map(k => Pubkey(k));
    // 3. Balance changes
    const balanceChanges = computeBalanceChanges(meta.pre_balances, meta.post_balances, skipZeroBalanceChanges);
 ...
 * @since 1.0.0
 */
export function parseAccountUpdate(raw, config = {}, timestamp) {
 ...
 * @since 1.0.0
 */
export function parseSlotUpdate(raw, timestamp) {
    const statusMap = {
        0: 'processed',
        1: 'confirmed',
        2: 'finalized',
    ...
    return {
        ...
        status: (statusMap[raw.status] ?? String(raw.status)),
        timestamp,
    };
}
/**
 ...
 * @since 1.0.0
 */
export function parseBlockMeta(raw, timestamp) {
    return {
        slot: Slot(Number(raw.slot)),
 ...
 * @since 1.0.0
 */
export function parseEntry(raw, timestamp) {
    return {
        slot: Slot(Number(raw.slot)),
 ...
 * @since 1.0.0
 */
export function parseGeyserUpdate(raw, config = {}) {
